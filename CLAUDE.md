...
- Clicks the "Forward" button by aria-label — **synthetic keyboard events don't work** because Gmail filters `isTrusted: false` events.
...
- **`isTrusted` filter**: Gmail ignores synthetic `KeyboardEvent`s for shortcuts. Always click real DOM buttons instead of simulating keypresses.
